  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.content}>
        ...
        <Section title="Debug">
          <Row label="API base URL" value={currentUrl} />
          <Row label="Tenant" value={session?.user.tenantSlug ?? '—'} />
          <Row label="User ID" value={session?.user.userId ?? '—'} />
          <Row label="Role" value={session?.user.role ?? '—'} />
          <Row label="Network" value={networkLabel} />
          <Row label="Offline queue" value={`${pendingCount} pending`} />
          <Row label="Emulator API" value="http://10.0.2.2:3001/v1" />
          <Row label="Env note" value="Web: localhost:3001/v1 · Mobile: LAN IP or 10.0.2.2" />
        </Section>
